...
function tooltip(name, result, year) {
...
export default function StateMap({ geography, resultsByState, year, selectedState, onSelect, states }) {
 ...
 const name=geoState(geo), active=states.includes(name), result=active?resultsByState[name]:null, score=result?.status==='full'?result.value:null, description=tooltip(name,result,year);
 return <g key={geo.rsmKey}><Geography geography={geo} fill={colorForScore(score)} stroke={selectedState===name?'#142f29':'#fff'} strokeWidth={selectedState===name?2:0.7} tabIndex={active?0:-1} role="button" aria-label={description.replaceAll('\n', '. ')} onClick={()=>active&&onSelect(name)} onKeyDown={e=>{if(active&&(e.key==='Enter'||e.key===' ')){e.preventDefault();onSelect(name);}}} style={{default:{outline:'none',opacity:active?1:.35},hover:{outline:'none',fill:'#9ab6a6',cursor:active?'pointer':'default'},pressed:{outline:'none'}}}><title>{description}</title></Geography>{score!=null&&<Marker coordinates={geoCentroid(geo)}><text textAnchor="middle" className="map-score" pointerEvents="none">{score.toFixed(0)}</text></Marker>}</g>;
