...
export default function App() {
  ...
  return <><Header metadata={data.metadata} tab={tab} setTab={setTab}/><main className="page">
    <div className="hero"><div><span className="eyebrow">SUSTAINABLE TOURISM / MALAYSIA</span><h1>Growth, with a longer view.</h1><p>Explore how domestic tourism, communities and coastal ecosystems move together.</p></div><a className="button download" href={`${import.meta.env.BASE_URL}data/Data.csv`} download>Download data <span>↗</span></a></div>
    ...
    <p className="filter-note">Year range applies to state and national trends. State selection also applies to forecasts. Headline KPIs always show the latest available national or coastal data.</p>
    {tab !== 'States' && <KpiCards data={data} overview={tab === 'Overview'}/>}<div className="content-stack">
